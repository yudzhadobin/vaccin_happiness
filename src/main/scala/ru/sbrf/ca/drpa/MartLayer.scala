...
object MartLayer {
  ...
  def run()(implicit session: SparkSession): Unit = {
    ...
    val error_vaccines_country = session.sql("SELECT * FROM happiness_2019 where Country_or_region not in (select xcountry from country_vaccinations)")
    val error_happiness_country = session.sql("SELECT * FROM country_vaccinations where xcountry not in (select Country_or_region from happiness_2019)")
